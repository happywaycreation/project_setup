...
async function fetchAddresses(userEmail) {
    try {
        const response = await fetch('https://happywaycreations.52.66.203.206.nip.io:3000/get-address', {
            method: 'POST',
            headers: {
    ...
    if(address.is_default === 0)
        {
            ...
            clone.querySelector(".set-default").addEventListener('click', (event) => {
                ...
                fetch('https://happywaycreations.52.66.203.206.nip.io:3000/set-default-address', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
