...
fetchWishProducts();
  ...
  const checkAndStoreCoupon = async (couponCode, appliedCoupons) => {
    try {
      ...
      if (appliedCoupons.includes(couponCode)) {
        ...
        const response = await fetch('https://52.66.203.20:3000/insert-coupon', {
          method: 'POST',
          headers: {
  ...
  const handleCouponApplication = async () => {
    ...
    try {
      // Fetch all coupons
      const couponsResponse = await fetch('https://52.66.203.20:3000/get-coupons', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
      ...
      const appliedCouponsResponse = await fetch('https://52.66.203.20:3000/get-applied-coupons-temp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  document.querySelector('.proceed-to-pay').addEventListener('click', async () => {
    ...
    if (cartProducts.length === 0) {
      ...
      try {
        ...
        const productResponse = await fetch('https://52.66.203.20:3000/get-cart-buy', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        let userEmail = sessionStorage.getItem('userEmail');
        const appliedCouponsResponse = await fetch('https://52.66.203.20:3000/get-applied-coupons-discount', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
