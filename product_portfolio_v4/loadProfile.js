const userEmail = sessionStorage.getItem('userEmail');
    ...
    async function loadProfile(email) {
        try {
            const response = await fetch(`http://13.126.114.64:3000/user-details?email=${encodeURIComponent(email)}`);
            if (!response.ok) {
    ...
    document.getElementById('editButton').addEventListener('click', async (evt) => {
        ...
        try {
            const response = await fetch('http://13.126.114.64:3000/update-user-details', {
                method: 'POST',
                headers: {
