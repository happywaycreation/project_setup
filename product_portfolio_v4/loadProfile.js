const userEmail = sessionStorage.getItem('userEmail');
    ...
    async function loadProfile(email) {
        try {
            const response = await fetch(`https://happywaycreations.52.66.203.206.nip.io:3000/user-details?email=${encodeURIComponent(email)}`);
            if (!response.ok) {
    ...
    document.getElementById('editButton').addEventListener('click', async (evt) => {
        ...
        try {
            const response = await fetch('https://happywaycreations.52.66.203.206.nip.io:3000/update-user-details', {
                method: 'POST',
                headers: {
