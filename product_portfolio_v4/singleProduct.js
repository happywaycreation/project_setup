...
const fetchProductById = async (productId) => {
    try {
        const response = await fetch(`https://happywaycreations.52.66.203.206.nip.io:3000/products/${productId}`);
        if (!response.ok) {
...
const fetchProductReviews = async (productId) => {
    try {
        const response = await fetch(`https://happywaycreations.52.66.203.206.nip.io:3000/getProductReview/${productId}`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        return await response.json();
